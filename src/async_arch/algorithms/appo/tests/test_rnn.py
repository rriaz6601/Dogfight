...
from src.async_arch.algorithms.appo.learner import (
...
class TestPackedSequences(TestCase):
    ...
    def check_packed_version_matching_loopy_version(self, T, N, D, random_dones):
        ...
        for _ in range(100):
            ...
            loopy_out = []
            for t in range(T):
                rnn_out, rnn_hidden_states = rnn(
                    x[t::T].view(1, N, -1), rnn_hidden_states
                )
            ...
            self.assertTrue(
                np.allclose(
                    packed_out.detach().numpy(), loopy_out.detach().numpy(), atol=2e-6
                )
            )
