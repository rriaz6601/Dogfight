"""This module is used to collect experience from the environment.

There are multiple classes in this module:
1. ActorState: It is the state of a single actor.
2. VectorEnvRunner: Actually runs an environment.
"""
...
from src.async_arch.algorithms.appo.appo_utils import TaskType
from src.async_arch.algorithms.appo.policy_manager import PolicyManager
from src.async_arch.algorithms.appo.population_based_training import PbtTask
from src.async_arch.algorithms.appo.shared_buffers import SharedBuffers, TensorDict
from src.async_arch.algorithms.utils.spaces.discretized import Discretized
from src.async_arch.utils.env_utils import (
    DictObservationsWrapper,
    MultiAgentWrapper,
...
from src.async_arch.utils.gpu_utils import set_gpus_for_process
...
def transform_dict_observations(observations: List):
...
class ActorState:
    ...
    Single-agent environments are treated as multi-agent with one agent for
    simplicity.
    ...
    curr_traj_buffer: TensorDict
    last_actions: Any
    last_rnn_state: np.ndarray

    def __init__(
        self,
        cfg,
        env: Union[MultiAgentWrapper, DictObservationsWrapper],
        worker_idx: int,
        split_idx: int,
        env_idx: int,
        agent_idx: int,
        ...
        ----------
        cfg:
            json dictionary of all the defined arguments in various classes
        env:
            the gym based RL environment (TODO: instance or class itself)
        worker_idx:
            which number of worker is bound to this environment
        split_idx: int
            TODO: what is split?
        env_idx: int
            It refers to a specific instance of env.
        agent_idx: int
            The agent which this state belongs to
        shared_buffers:
            Of the type shared buffer. (still need to understand what are
                these for)
        policy_output_tensors:
            TODO: maybe the results?
        pbt_reward_shaping:
            TODO: population based reward.
        policy_mgr:
            Mapping from policy to environment, currently it is just a random
            selection.
        ...
        self.curr_traj_buffer_idx = None
        ...
        # whether this agent received actions from the policy and can act in
        # the environment again
        self.ready = False

        # By returning info = {'is_active': False, ...} the environment can
        # indicate that the agent is not active, i.e. dead or otherwise
        # disabled. Experience from such agents will be ignored.
        self.is_active = True

        # This flag is reset at the beginning of every rollout. If the agent
        # was inactive during the entire rollout then it is ignored and no
        # experience is sent to the learner.
        self.has_rollout_data = False

        # whether this actor requires a new trajectory buffer
        self.needs_buffer = False

        self.num_trajectories = 0

        # have to count env steps per policy, since a single rollout may
        # contain experience from more than one policy
        ...
        # dictionary with current training progress per policy values are
        # approximate because we get updates from the master process once every
        # few seconds
        ...
        if isinstance(env.action_space, Tuple):
            ...
            if all_subspaces_discrete:
                ...
                # technically possible to add support for such spaces, but it's
                # untested for now, look at Discretized instead.
                raise Exception(
                    "Mixed discrete & continuous action spaces are not supported"
                    "(should be an easy fix)"
                )

        self.env_training_info_interface = find_training_info_interface(env)

    def _env_set_curr_policy(self) -> None:
        """Set the current policy that is collecting experience.

        Most environments do not need to know index of the policy that
        currently collects experience. But in rare cases it is necessary.
        Originally was implemented for DMLab to properly manage the level cache.

        Can be useful for analysing which policies were used the most in case
        of non-random selections but this functionality maybe better in the
        selection function rather than here.
    ...
    def _on_new_policy(self, new_policy_id):
        ...
        # policy change can only happen at the episode boundary so no need to
        # reset rnn state (but I guess does not hurt)
        self._reset_rnn_state()

        if (
            self.cfg.with_pbt
            and self.pbt_reward_shaping[self.curr_policy_id] is not None
        ):
            set_reward_shaping(
                self.env, self.pbt_reward_shaping[self.curr_policy_id], self.agent_idx
            )
            set_training_info(
                self.env_training_info_interface,
    ...
    def update_traj_buffer(self, traj_buffer_idx):
        ...
        self.curr_traj_buffer: TensorDict = self.shared_buffers.tensors.index(
            self.curr_traj_buffer_idx
        )
        self.needs_buffer = False

    def set_trajectory_data(self, data, rollout_step):
        """Write a dictionary of data into a trajectory buffer at the specific
        location (rollout_step).

        Parameters
        ----------
        data :
            any sub-dictionary of the full per-step data, e.g. just
            observation, observation and action, etc.
        rollout_step :
            number of steps since we started the current rollout. When this
            reaches cfg.rollout we finalize the trajectory buffer and send it
            to the learner.
    ...
    def curr_actions(self):
        """:return: the latest set of actions for this actor, calculated by the
        policy worker for the last observation
    ...
    def record_env_step(self, reward, done, info, rollout_step):
        """Policy inputs (obs) and policy outputs (actions, values, ...) for
        the current rollout step are already added to the trajectory buffer
        the only job remaining is to add auxiliary data: rewards, done flags, etc.

        Parameters
        ----------
        reward :
            last reward from the env step
        done :
            last value of done flag
        info :
            info dictionary
        rollout_step :
            number of steps since we started the current rollout. When this
        ...
        if done:
            ...
            report = self.episodic_stats(
                last_episode_true_reward, last_episode_extra_stats
            )
            ...
            new_policy_id = self.policy_mgr.get_policy_for_agent(
                self.agent_idx, self.env_idx
            )
    ...
    def finalize_trajectory(self, rollout_step):
        ...
        ----------
        rollout_step :
            number of steps since we started the current rollout. This should be equal to
        ...
        if not self.has_rollout_data:
            # this agent was inactive the entire rollout, send no trajectories
            # to the learners
            return []

        self.has_rollout_data = False

        # We could change policy id in the middle of the rollout (i.e. on the
        # episode boundary), in which case this trajectory should be sent to
        # two learners, one for the original policy id, one for the new one.
        # The part of the experience that belongs to a different policy will be
        # ignored on the learner.
        trajectories = []
        buffers_used = set()
        ...
        for policy_id in np.unique(self.curr_traj_buffer["policy_id"]):
            ...
            if policy_id == -1:
                # -1 is a policy that does not exist, used to mark inactive
                # -agents not controlled by any policy
                continue

            traj_buffer_idx = self.curr_traj_buffer_idx
            if traj_buffer_idx in buffers_used:
                # This rollout needs to be sent to multiple learners, i.e.
                # because the policy changed in the middle of the rollout. If
                # we use the same shared buffer on multiple learners, we need
                # some mechanism to guarantee that this buffer will only be
                # released once. It seems easier to just copy all data to a new
                # buffer for each additional learner. This should be a very
                # rare event so the performance impact is negligible.
                traj_buffer_idx = self.shared_buffers.get_trajectory_buffers(
                    num_buffers=1
                )[0]
                buffer = self.shared_buffers.tensors.index(traj_buffer_idx)
                buffer.set_data(
                    slice(None), self.curr_traj_buffer
                )  # copy TensorDict data recursively
            ...
            traj_dict = dict(
                t_id=t_id,
                length=rollout_step,
                env_steps=self.rollout_env_steps[policy_id],
                policy_id=policy_id,
    ...
    def episodic_stats(self, last_episode_true_reward, last_episode_extra_stats):
        ...
        Parameters
        ----------
        last_episode_true_reward :
            param last_episode_extra_stats:
        last_episode_extra_stats :
...
class VectorEnvRunner:
    ...
    With double buffering each actor worker holds two vector runners and
    switches between them. Without single buffering we only use a single
    VectorEnvRunner per actor worker.

    All envs on a single VectorEnvRunner run in unison, e.g. they all do one
    step at a time together. This also means they all finish their rollouts
    together. This allows us to minimize the amount of messages
    passed around.

    Individual envs (or agents in these envs in case of multi-agent) can
    potentially be controlled by different policies when we're doing PBT. We
    only start simulating the next step in the environment when all actions
    from all envs and all policies are collected. This leaves optimization
    potential: we can start simulating some envs right away as actions for them
    arrive. But usually double-buffered sampling masks this type of
    inefficiency anyway. The worker is probably still rendering a previous
    vector of envs when the actions arrive.

    Parameters
    ----------
    cfg :
        global system config (all CLI params)
    num_envs :
        number of envs to run in this vector runner
    worker_idx :
        idx of the parent worker
    split_idx :
        index of the environment group in double-buffered sampling (either 0 or
        1). Always 0 when double-buffered sampling is disabled.
    num_agents :
        number of agents in each env (1 for single-agent envs)
    shared_buffers :
        a collection of all shared data structures used by the algorithm. Most
        importantly, the trajectory buffers in shared memory.
    pbt_reward_shaping :
        initial reward shaping dictionary, for configuration where PBT
        optimizes reward coefficients in environments.
    ...
    def __init__(
        ...
        self.shared_buffers = shared_buffers
        self.policy_output_tensors = self.shared_buffers.policy_output_tensors[
    ...
    def init(self):
        ...
        Also creates ActorState objects that hold the state of individual
        actors in (potentially) multi-agent envs.
    ...
    def _process_policy_outputs(self, policy_id, timing):
        """Process the latest data from the policy worker (for policy =
        policy_id).

        Policy outputs currently include new RNN states, actions, values,
        logprobs, etc. See shared_buffers.py for the full list of outputs.

        As a performance optimization, all these tensors are squished together
        into a single tensor. This allows us to copy them to shared memory only
        once, which makes a difference on the policy worker. Here we do
        np.split to separate them back into individual tensors.

        Parameters
        ----------
        policy_id :
            index of the policy whose outputs we're currently processing
        timing :
        ...
        for env_i in range(self.num_envs):
            for agent_i in range(self.num_agents):
                ...
                if actor_policy == policy_id:
                    ...
                    actor_state.set_trajectory_data(
                        policy_outputs_dict, self.rollout_step
                    )
    ...
    def _process_env_step(self, new_obs, rewards, dones, infos, env_i):
        ...
        ----------
        new_obs :
            latest observations from the env
        env_i :
            index of the environment in the vector
        rewards :
            param dones:
        infos :
    ...
    def _finalize_trajectories(self):
        ...
        for env_i in range(self.num_envs):
            for agent_i in range(self.num_agents):
                rollouts.extend(
                    self.actor_states[env_i][agent_i].finalize_trajectory(
                        self.rollout_step
                    )
                )
    ...
    def _update_trajectory_buffers(self, timing):
        ...
        if num_buffers_to_request > 0:
            traj_buffer_indices = self.shared_buffers.get_trajectory_buffers(
                num_buffers_to_request, timing
            )
    ...
    def _format_policy_request(self):
        """Format data that allows us to request new actions from policies that
        control the agents in all the envs.

        Note how the data required is basically just indices of envs and
        agents, as well as location of the step data in the shared rollout
        buffer. This is enough for the policy worker
        to find the step data in the shared data structure.
            ...
            for agent_i in range(self.num_agents):
                ...
                if actor_state.is_active:
                    ...
                    data = (
                        env_i,
                        agent_i,
                        actor_state.curr_traj_buffer_idx,
                        self.rollout_step,
                    )
    ...
    def _prepare_next_step(self):
        """Write environment outputs to shared memory so policy workers can
        calculate actions for the next step.

        Note how we temporarily hold obs and rnn_states in local variables
        before writing them into shared memory. We could not do the memory
        write right away because for that we need the memory location of the
        NEXT step.

        If this is the first step in the new rollout, we need to switch to a
        new trajectory buffer before we do that (because the previous
        trajectory buffer is now used by the learner and we can't use it until
        the learner is done).
        ...
        for env_i in range(self.num_envs):
            for agent_i in range(self.num_agents):
                ...
                if actor_state.is_active:
                    ...
                    policy_inputs = dict(
                        obs=actor_state.last_obs, rnn_states=actor_state.last_rnn_state
                    )
    ...
    def reset(self, report_queue):
        """Do the very first reset for all environments in a vector. Populate
        shared memory with initial obs.

        Note that this is called only once, at the very beginning of training.
        After this the envs should auto-reset.

        Parameters
        ----------
        report_queue :
            we use report queue to monitor reset progress (see appo.py). This can be a lengthy
            process.
        ...
        for env_i, e in enumerate(self.envs):
            ...
            if self.cfg.decorrelate_envs_on_one_worker:
                ...
                decorrelate_steps = (
                    self.cfg.rollout * env_i_split
                    + self.cfg.rollout * random.randint(0, 4)
                )
    ...
    def advance_rollouts(self, data, timing):
        """Main function in VectorEnvRunner.

        Does one step of simulation (if all actions for all actors are available).

        Parameters
        ----------
        data :
            incoming data from policy workers (policy outputs), including new actions
        timing :
...
class ActorWorker:
    ...
    2. Send observations to a queue for action generation elsewhere (e.g. on a
    GPU worker)
    3. Immediately start processing second half of the vector (envs N/2+1..N)
    4. By the time second half is processed, actions for the 1st half should be
    ready. Immediately start processing the 1st half of the vector again.

    As a result, if action generation is fast enough, this env runner should be
    busy 100% of the time calculating env steps, without waiting for actions.
    ...
    ----------
    cfg :
        global config (all CLI params)
    obs_space :
        observation space (spaces) of the environment
    action_space :
        action space for the agent
    num_agents :
        number of agents per env (all env should have the same number of agents
        right now, although it should be easy to fix)
    worker_idx :
        index of this worker process
    shared_buffers :
        shared memory data structures initialized in main process (see
    shared_buffers.py) :

    task_queue :
        queue for incoming messages for THIS particular actor worker. See the
        task types in the loop below, but the most common task is ROLLOUT_STEP,
        which means "here's your actions, advance simulation by one step.
    policy_queues :
        FIFO queues associated with all policies participating in training. We
        send requests for policy queue #N to get actions for envs (agents) that
        are controlled by policy #N.
    report_queue :
        one-way communication with the main process, various stats and whatnot
    learner_queues :
        one-way communication with the learner, sending trajectory buffers for learning
    ...
    -------
    init:
        request the actor to start the task by putting in INIT flag in
        task_queue. This launches the _init() method, which deals with the
        initiation of the env runners and communication with it.
    request_reset:
        put the RESET flag to the actor task_queue. This launches
        _handle_reset() method to reset the environment in the and receive the
        first observation. Then sending the time to record queue.
    update_env_steps:
        Called by main loop, TODO: when workers are always running why call
        this method? My guess is that it tells all the actors upto what
        env_step we are. I am not sure as to what use it can be.
    """
    ...
    def __init__(
        ...
        assert (
            self.vector_size % self.num_splits == 0
        ), "Vector size should be divisible by num_splits"
    ...
    def _init(self):
        """Initialize env runners, that actually do all the work.

        Also we're doing some utility stuff here:
        1. setting thread limit of environments to one,
        2. setting process affinity (this is a performance optimization).
    ...
    def _terminate(self):
        """Terminate the environments associated with this worker."""
    ...
    def _handle_reset(self):
        """Reset all envs, one split at a time (double-buffering), and send
        requests to policy workers to get actions for the very first env step.
        ...
        safe_put(
            self.report_queue, dict(finished_reset=self.worker_idx), queue_name="report"
        )

    def _advance_rollouts(self, data, timing):
        """Process incoming request from policy worker. Use the data (policy
        outputs, actions) to advance the simulation by one step on the
        corresponding VectorEnvRunner.

        If we successfully managed to advance the simulation, send requests to
        policy workers to get actions for the next step. If we completed the
        entire rollout, also send request to the learner!
        ...
        with timing.add_time("complete_rollouts"):
            if complete_rollouts:
                ...
                if self.num_complete_rollouts == 0 and not self.cfg.benchmark:
                    # we just finished our first complete rollouts, perfect
                    # time to wait for experience derorrelation this guarantees
                    # that there won't be any obsolete trajectories when we
                    # awaken
                    delay = (
                        float(self.worker_idx) / self.cfg.num_workers
    ...
    def _process_pbt_task(self, pbt_task):
        """Save the latest version of reward shaping from PBT, we later
        propagate this to envs.
    ...
    def _run(self):
        ...
        Process tasks (mainly ROLLOUT_STEP) until we get the termination
        signal, which usually means end of training. Currently there is no
        mechanism to restart dead workers if something bad happens during
        training. We can only retry on the initial reset(). This is definitely
        ...
        log.info(
            f"ACTOR worker {self.worker_idx}\tpid {os.getpid()}\tparent {os.getppid()}"
        )

        # workers should ignore Ctrl+C because the termination is handled in
        # the event loop by a special msg
            ...
            while not self.terminate:
                try:
                    ...
                    if time.time() - last_report > 5.0 and "one_step" in timing:
                        timing_stats = dict(
                            wait_actor=timing.wait_actor, step_actor=timing.one_step
                        )
    ...
    def init(self):
        """Receive init flag from the main process."""
        self.task_queue.put((TaskType.INIT, None))

    def request_reset(self):
        """Main process requests actor to reset the environment"""
