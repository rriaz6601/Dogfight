...
from src.async_arch.algorithms.appo.appo_utils import (
    copy_dict_structure,
    iter_dicts_recursively,
    iterate_recursively,
)
from src.async_arch.algorithms.appo.model_utils import get_hidden_size
from src.async_arch.algorithms.utils.action_distributions import (
    calc_num_actions,
    calc_num_logits,
)
...
class SharedBuffers:
    """Shared buffers for transmitting trajectories and model.
    ...
    ----------
    policy_outputs
        Sorted list of output types from the policy, all elements have a name
        and size attribute. These elements include [action, action_logits,
        log_prob_action, values, policy_version and rnn_states].
    Methods
    -------
    get_trajectory_buffers(num_buffers, timing)
        N buffers to store trajectory and optional timing to benchmark.
    """
    ...
    def __init__(self, cfg, num_agents, obs_space, action_space):
        ...
        # this is for performance optimization indexing in numpy arrays is
        # faster than in PyTorch tensors
        self.tensors: TensorDict = self.tensor_dict_to_numpy()

        # copying small policy outputs (e.g. individual value predictions &
        # action logits) to shared memory is a bottleneck on the policy worker.

        # For optimization purposes we create additional tensors to hold just
        # concatenated policy outputs. Rollout workers parse the data and add
        # it to the trajectory buffers in a proper format
        ...
        # a list of boolean flags to be shared among components that indicate
        # that experience collection should be temporarily stopped (e.g. due to
        # too much experience accumulated on the learner)
        ...
        queue_max_size_bytes = self.num_traj_buffers * 40
        self.free_buffers_queue = faster_fifo.Queue(max_size_bytes=queue_max_size_bytes)

        # since all buffers are initially free, we add all buffer indices to
        # the queue
    ...
    def calc_num_trajectory_buffers(self):
        """This calculates the number of shared trajectory (rollout) buffers
        required by the system to operate without interruptions.

        This consists of:
        1) at least one trajectory buffer per agent, such that we always have a
        location to save new experience
        2) a few trajectory buffers to hold data currently processed by the
        learner (including potential backlog)
        3) (potentially) some extra trajectory buffers to keep the system
        operational. These might be required i.e. in multi-agent envs when some
        agents are deactivated during the rollout. Such agents together with
        the learner can hold on to many buffers, such that active agents might
        not have enough free buffers to continue collecting the experience.
        ...
        max_experience_on_learners = max_minibatches_to_accumulate * (
            self.cfg.batch_size * self.cfg.num_policies
        )
    ...
    def tensor_dict_to_numpy(self):
        """Convert tensor dict to a numpy dictionary."""
    ...
    def get_trajectory_buffers(self, num_buffers: int, timing: Optional[Timing] = None):
        """Obtain shared buffer to store a trajectory.

        Parameters
        ----------
        num_buffers :
            number of free buffer indices to obtain
        timing :
...
class TensorDict(dict):
    ...
    def index_func(self, x, indices):
        ...
        ----------
        x :
            A TensorDict like object which might have nested dictionaries.
        indices :
