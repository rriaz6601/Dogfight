...
from src.async_arch.algorithms.appo.aux_losses import CPCA
from src.async_arch.algorithms.appo.model import create_actor_critic
...
from src.async_arch.algorithms.utils.algo_utils import EPS, calculate_gae
from src.async_arch.algorithms.utils.pytorch_utils import to_scalar
from src.async_arch.utils.decay import LinearDecay
from src.async_arch.utils.gpu_utils import cuda_envvars_for_policy
...
def build_rnn_inputs(x, dones_cpu, rnn_states, T: int):
    ...
    ----------
    x :
        A (N*T, -1) tensor of the data to build the PackedSequence out of
    dones_cpu :
        A (N*T) tensor where dones[i] == 1.0 indicates an episode is done, a CPU-bound tensor
    rnn_states :
        A (N*T, -1) tensor of the rnn_hidden_states
    T :
...
class LearnerWorker:
    ...
    train_thread_initialized: threading.Event
    ...
    def __init__(
        ...
        self.tensor_batch_pool = None
        # self.tensor_batcher = None
        ...
        # decay rate at which summaries are collected: save summaries every 5
        # seconds in the beginning, but decay to every 4 minutes in the limit,
        # because we do not need frequent summaries for longer experiments
