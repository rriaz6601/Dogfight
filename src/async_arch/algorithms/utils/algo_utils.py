...
def calculate_discounted_sum(x, dones, discount, x_last=None):
    ...
    ----------
    x :
        ndarray of shape [num_steps, num_envs]
    dones :
        ndarray of shape [num_steps, num_envs]
    discount :
        float in range [0,1]
    x_last :
        iterable of shape [num_envs], value at the end of trajectory. None interpreted as zero(s). (Default value = None)
...
def calculate_gae(rewards, dones, values, gamma, gae_lambda):
    ...
    ----------
    rewards :
        actual environment rewards
    dones :
        True if absorbing state is reached
    values :
        estimated values
    gamma :
        discount factor [0,1]
    gae_lambda :
        lambda-factor for GAE (discounting for longer-horizon advantage estimations), [0,1]
