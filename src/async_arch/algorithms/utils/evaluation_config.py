...
def add_eval_args(parser):
    ...
    """
    # TODO: Need to tailor this experience for my environments
    parser.add_argument(
        "--fps",
        default=0,
        type=int,
        help="""Enable sync mode with adjustable FPS. Default (0) means
        default, e.g. for Doom its FPS (~35), or unlimited if not specified by
        env. Leave at 0 for Doom multiplayer evaluation""",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        type=int,
        help="""Repeat an action that many frames during evaluation. By default
        uses the value from env config (used during training).""",
    )
