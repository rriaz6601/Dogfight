...
def get_action_distribution(action_space, raw_logits):
    ...
    raw_logits :
        this function expects unprocessed raw logits (not after log-softmax!)

    Returns
    -------
    type
        action distribution that you can sample from
...
class CategoricalActionDistribution:
    ...
    ----------
    raw_logits :
        unprocessed logits, typically an output of a fully-connected layer
...
class TupleActionDistribution:
    ...
    Empirically, it seems to be better to represent such an action distribution
    as a tuple of independent action distributions, rather than a one-hot over
    potentially big cartesian product of all action spaces, like it's usually
    done in Atari.
