...
class JsbsimCatalog(Property, Enum):
    ...
    position_lat_geod_deg = Property(
        "position/lat-geod-deg", "geocentric latitude [deg]", -90, 90, access="R"
    )
    ...
    position_distance_from_start_lat_mt = Property(
        "position/distance-from-start-lat-mt", "mt", access="R"
    )
    position_distance_from_start_lon_mt = Property(
        "position/distance-from-start-lon-mt", "mt", access="R"
    )
    position_epa_rad = Property("position/epa-rad", "rad", access="R")
    position_geod_alt_ft = Property("position/geod-alt-ft", "ft", access="R")
    position_radius_to_vehicle_ft = Property(
        "position/radius-to-vehicle-ft", "ft", access="R"
    )
    position_terrain_elevation_asl_ft = Property(
        "position/terrain-elevation-asl-ft", "ft"
    )

    # velocities

    velocities_u_fps = Property(
        "velocities/u-fps", "body frame x-axis velocity [ft/s]", -2200, 2200, access="R"
    )
    velocities_v_fps = Property(
        "velocities/v-fps", "body frame y-axis velocity [ft/s]", -2200, 2200, access="R"
    )
    velocities_w_fps = Property(
        "velocities/w-fps", "body frame z-axis velocity [ft/s]", -2200, 2200, access="R"
    )
    velocities_v_north_fps = Property(
        "velocities/v-north-fps", "velocity true north [ft/s]", -2200, 2200, access="R"
    )
    velocities_v_east_fps = Property(
        "velocities/v-east-fps", "velocity east [ft/s]", -2200, 2200, access="R"
    )
    velocities_v_down_fps = Property(
        "velocities/v-down-fps", "velocity downwards [ft/s]", -2200, 2200, access="R"
    )
    velocities_vc_fps = Property(
        "velocities/vc-fps", "airspeed in knots", 0, 4400, access="R"
    )
    velocities_h_dot_fps = Property(
        "velocities/h-dot-fps", "rate of altitude change [ft/s]", access="R"
    )
    ...
    velocities_eci_velocity_mag_fps = Property(
        "velocities/eci-velocity-mag-fps", "fps", access="R"
    )
    ...
    velocities_p_aero_rad_sec = Property(
        "velocities/p-aero-rad_sec", "rad/sec", access="R"
    )
    velocities_q_aero_rad_sec = Property(
        "velocities/q-aero-rad_sec", "rad/sec", access="R"
    )
    velocities_r_aero_rad_sec = Property(
        "velocities/r-aero-rad_sec", "rad/sec", access="R"
    )
    velocities_phidot_rad_sec = Property(
        "velocities/phidot-rad_sec", "rad/s", -2 * math.pi, 2 * math.pi, access="R"
    )
    velocities_thetadot_rad_sec = Property(
        "velocities/thetadot-rad_sec", "rad/s", -2 * math.pi, 2 * math.pi, access="R"
    )
    velocities_psidot_rad_sec = Property(
        "velocities/psidot-rad_sec", "rad/sec", -2 * math.pi, 2 * math.pi, access="R"
    )
    ...
    accelerations_vdot_ft_sec2 = Property(
        "accelerations/vdot-ft_sec2", "ft/sÂ²", -4.0, 4.0, access="R"
    )
    accelerations_wdot_ft_sec2 = Property(
        "accelerations/wdot-ft_sec2", "ft/sÂ²", -4.0, 4.0, access="R"
    )
    accelerations_udot_ft_sec2 = Property(
        "accelerations/udot-ft_sec2", "ft/sÂ²", -4.0, 4.0, access="R"
    )
    accelerations_a_pilot_x_ft_sec2 = Property(
        "accelerations/a-pilot-x-ft_sec2",
        "pilot body x-axis acceleration [ft/sÂ²]",
        access="R",
    )
    accelerations_a_pilot_y_ft_sec2 = Property(
        "accelerations/a-pilot-y-ft_sec2",
        "pilot body y-axis acceleration [ft/sÂ²]",
        access="R",
    )
    accelerations_a_pilot_z_ft_sec2 = Property(
        "accelerations/a-pilot-z-ft_sec2",
        "pilot body z-axis acceleration [ft/sÂ²]",
        access="R",
    )
    accelerations_n_pilot_x_norm = Property(
        "accelerations/n-pilot-x-norm",
        "pilot body x-axis acceleration, normalised",
        access="R",
    )
    accelerations_n_pilot_y_norm = Property(
        "accelerations/n-pilot-y-norm",
        "pilot body y-axis acceleration, normalised",
        access="R",
    )
    accelerations_n_pilot_z_norm = Property(
        "accelerations/n-pilot-z-norm",
        "pilot body z-axis acceleration, normalised",
        access="R",
    )
    ...
    def update_equal_engine_props(sim, prop):
        ...
        for i in range(1, n):
            sim.jsbsim_exec.set_property_value(
                prop.name_jsbsim + "[" + str(i) + "]", value
            )
    ...
    def update_equal_throttle_pos(sim):
        ...
        """
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_throttle_pos_norm
        )
    ...
    fcs_speedbrake_pos_norm = Property(
        "fcs/speedbrake-pos-norm", "speedbrake position, normalised", 0, 1
    )
    ...
    fcs_throttle_pos_norm = Property(
        ...
        "throttle position, normalised",
        0,
        1,
        update=update_equal_throttle_pos,
    )
    fcs_mixture_pos_norm = Property(
        "fcs/mixture-pos-norm", "normalised", update=update_equal_mixture_pos
    )
    gear_gear_pos_norm = Property(
        "gear/gear-pos-norm", "landing gear position, normalised", 0, 1
    )
    gear_num_units = Property("gear/num-units", "number of gears", access="R")
    fcs_feather_pos_norm = Property(
        "fcs/feather-pos-norm", "normalised", update=update_equal_feather_pos
    )
    fcs_advance_pos_norm = Property(
        "fcs/advance-pos-norm", "normalised", update=update_equal_advance_pos
    )
    ...
    def update_equal_throttle_cmd(sim):
        ...
        """
        JsbsimCatalog.update_equal_engine_props(
            sim, JsbsimCatalog.fcs_throttle_cmd_norm
        )
    ...
    def update_equal_brake_props(sim):
        ...
        sim.jsbsim_exec.set_property_value(
            JsbsimCatalog.fcs_left_brake_cmd_norm.name_jsbsim, value
        )
        sim.jsbsim_exec.set_property_value(
            JsbsimCatalog.fcs_right_brake_cmd_norm.name_jsbsim, value
        )
    ...
    fcs_aileron_cmd_norm = Property(
        "fcs/aileron-cmd-norm", "aileron commanded position, normalised", -1.0, 1.0
    )
    fcs_elevator_cmd_norm = Property(
        "fcs/elevator-cmd-norm", "elevator commanded position, normalised", -1.0, 1.0
    )
    fcs_rudder_cmd_norm = Property(
        "fcs/rudder-cmd-norm", "rudder commanded position, normalised", -1.0, 1.0
    )
    fcs_throttle_cmd_norm = Property(
        "fcs/throttle-cmd-norm",
        "throttle commanded position, normalised",
        0.0,
        0.9,
        update=update_equal_throttle_cmd,
    )
    fcs_mixture_cmd_norm = Property(
        "fcs/mixture-cmd-norm",
        "engine mixture setting, normalised",
        0.0,
        1.0,
        update=update_equal_mixture_cmd,
    )
    gear_gear_cmd_norm = Property(
        "gear/gear-cmd-norm",
        "all landing gear commanded position, normalised",
        0.0,
        1.0,
    )
    fcs_speedbrake_cmd_norm = Property("fcs/speedbrake-cmd-norm", "normalised")
    fcs_left_brake_cmd_norm = Property(
        "fcs/left-brake-cmd-norm", "Left brake command(normalized)", 0.0, 1.0
    )
    fcs_center_brake_cmd_norm = Property(
        ...
        0.0,
        1.0,
        update=update_equal_brake_cmd,
    )
    fcs_right_brake_cmd_norm = Property(
        "fcs/right-brake-cmd-norm", "Right brake command(normalized)", 0.0, 1.0
    )
    fcs_spoiler_cmd_norm = Property("fcs/spoiler-cmd-norm", "normalised")
    fcs_flap_cmd_norm = Property("fcs/flap-cmd-norm", "normalised")
    fcs_steer_cmd_norm = Property(
        "fcs/steer-cmd-norm", "Steer command(normalized)", -1.0, 1.0
    )
    fcs_advance_cmd_norm = Property(
        "fcs/advance-cmd-norm", "normalised", update=update_equal_advance_cmd
    )
    fcs_feather_cmd_norm = Property(
        "fcs/feather-cmd-norm", "normalised", update=update_equal_feather_cmd
    )
    ...
    ic_h_sl_ft = Property(
        "ic/h-sl-ft",
        "initial altitude MSL [ft]",
        position_h_sl_ft.min,
        position_h_sl_ft.max,
    )
    ic_h_agl_ft = Property(
        "ic/h-agl-ft", "", position_h_sl_ft.min, position_h_sl_ft.max
    )
    ...
    ic_psi_true_deg = Property(
        "ic/psi-true-deg",
        "initial (true) heading [deg]",
        attitude_psi_deg.min,
        attitude_psi_deg.max,
    )
    ...
    propulsion_engine_set_running = Property(
        "propulsion/engine/set-running", "engine running (0/1 bool)"
    )
    propulsion_set_running = Property(
        "propulsion/set-running", "set engine running (-1 for all engines)", access="W"
    )
    propulsion_tank_contents_lbs = Property("propulsion/tank/contents-lbs", "")
    propulsion_total_fuel_lbs = Property(
        "propulsion/total-fuel-lbs", "total remaining amount of fuel [lbs]"
    )

    # simulation

    simulation_dt = Property(
        "simulation/dt", "JSBSim simulation timestep [s]", access="R"
    )
    simulation_sim_time_sec = Property(
        "simulation/sim-time-sec", "Simulation time [s]", access="R"
    )
