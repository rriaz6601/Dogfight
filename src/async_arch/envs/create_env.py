...
def create_env(full_env_name, cfg=None, env_config=None):
    ...
    ----------
    full_env_name :
        complete name of the environment, starting with the prefix of registered environment family,
    ...
    cfg :
        namespace with full system configuration, output of argparser (or AttrDict when loaded from JSON) (Default value = None)
    env_config :
        AttrDict with additional system information:
        env_config = AttrDict(worker_index=self.worker_idx, vector_index=vector_idx, env_id=env_id) (Default value = None)
